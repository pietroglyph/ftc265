...
namespace Eigen {
    ...
    namespace internal {
        ...
        struct scalar_identity_op {
            ...
            const Scalar operator()(IndexType row, IndexType col) const {
            ...
            linspaced_op_impl(const Scalar &low, const Scalar &high, Index num_steps) :
                    ...
                    m_divisor(convert_index<Scalar>(
                            (high >= low ? num_steps : -num_steps) + (high - low)) /
                              ((numext::abs(high - low) + 1) == 0 ? 1 : (numext::abs(high - low) +
                                                                         1))),
...
// nested expressions).
        ...
        struct functor_traits<linspaced_op<Scalar, PacketType> > {
            enum {
                Cost = 1,
                PacketAccess =
                (!NumTraits<Scalar>::IsInteger) && packet_traits<Scalar>::HasSetLinear &&
