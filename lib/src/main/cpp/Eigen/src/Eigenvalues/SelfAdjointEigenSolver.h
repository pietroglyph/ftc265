...
namespace Eigen {
    ...
    namespace internal {
        ...
        ComputationInfo
        computeFromTridiagonal_impl(DiagType &diag, SubDiagType &subdiag, const Index maxIterations,
...
/** \eigenvalues_module \ingroup Eigenvalues_Module
  ...
  */
    ...
    public:
        ...
        EIGEN_DEVICE_FUNC
        explicit SelfAdjointEigenSolver(const EigenBase <InputType> &matrix,
        ...
        /**
          ...
          */
        SelfAdjointEigenSolver &
        computeFromTridiagonal(const RealVectorType &diag, const SubDiagonalType &subdiag,
                               int options = ComputeEigenvectors);
...
/** \internal
  ...
  */
        ...
        EIGEN_DEVICE_FUNC
        static void
        tridiagonal_qr_step(RealScalar *diag, RealScalar *subdiag, Index start, Index end,
    ...
    ::compute(const EigenBase <InputType> &a_matrix, int options) {
        ...
        m_info = internal::computeFromTridiagonal_impl(diag, m_subdiag, m_maxIterations,
    ...
    ::computeFromTridiagonal(const RealVectorType &diag, const SubDiagonalType &subdiag,
        ...
        m_info = internal::computeFromTridiagonal_impl(m_eivalues, m_subdiag, m_maxIterations,
                                                       computeEigenvectors, m_eivec);
...
/**
  ...
  */
        ...
        computeFromTridiagonal_impl(DiagType &diag, SubDiagType &subdiag, const Index maxIterations,
            ...
            while (end > 0) {
                for (Index i = start; i < end; ++i)
                    if (internal::isMuchSmallerThan(abs(subdiag[i]),
                                                    (abs(diag[i]) + abs(diag[i + 1])), precision) ||
                ...
                internal::tridiagonal_qr_step<
                        MatrixType::Flags & RowMajorBit ? RowMajor : ColMajor>(diag.data(),
                                                                               subdiag.data(),
                                                                               start, end,
                                                                               computeEigenvectors
                                                                               ? eivec.data()
        ...
        struct direct_selfadjoint_eigenvalues {
            EIGEN_DEVICE_FUNC
            static inline void run(SolverType &eig, const typename SolverType::MatrixType &A,
                                   int options) { eig.compute(A, options); }
        };
